...
/*
 * Handler for adding signatures to SOAP-requests
 */
...
public class AwsHandlerResolver implements HandlerResolver {
	...
	public AwsHandlerResolver(String accessKey, String awsSecretKey) {
	...
	public List<Handler> getHandlerChain(PortInfo portInfo) {
       ...
       if(serviceQName.getLocalPart().equals("AWSECommerceService")) {
    	   handlerChain.add(new AwsHandler(accessKey, awsSecretKey));
       }
	...
	private static class AwsHandler implements SOAPHandler<SOAPMessageContext> {
        ...
        public AwsHandler(String accessKey, String awsSecretKey) {
		...
		public boolean handleMessage(SOAPMessageContext messagecontext) {
			...
			if (outbound) {
				try {
			    	...
			    	appendTextElement(soapHeader, "Signature", signature);
			    	appendTextElement(soapHeader, "Timestamp", timeStamp);
			    	appendTextElement(soapHeader, "AWSAccessKeyId", accessKey);
		...
		private String getSignature(String operation, String timeStamp, byte[] secretBytes) {
			try {
				...
				String signature = new sun.misc.BASE64Encoder().encode(signBytes);
				return signature;
		...
		private void appendTextElement(Node node, String elementName, String elementText) {
	    	Element element = node.getOwnerDocument().createElementNS("http://security.amazonaws.com/doc/2007-01-01/", elementName);
